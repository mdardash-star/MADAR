...
const EMPTY: Omit<Supplier, 'id' | 'company_id' | 'is_active'> = {
  name: '',
  code: '',
  email: '',
  phone: '',
...
export default function SuppliersPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await listSuppliers(companyId, { search: search || undefined, limit: 100 });
      setRows(data);
      setError('');
    } catch (e) {
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-6xl">
        <PageHeader
          title="الموردون"
          subtitle={`${rows.length} مورد مسجل`}
          action={<AddButton onClick={openCreate}>إضافة مورد</AddButton>}
        />
        <div className="mb-4">
        ...
        {loading ? (
          ...
          <DataTable
            ...
            emptyMessage="لا يوجد موردون بعد. أضف مورداً جديداً."
          />
        )}
      </div>
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <SubmitButton loading={saving}>{editing ? 'حفظ التعديلات' : 'إضافة المورد'}</SubmitButton>
            </>
          }
        >
          <form id="supplier-form" onSubmit={handleSubmit} className="space-y-4">
            <Field label="اسم المورد *" required value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} />
            <Field label="الرمز (كود) *" required value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value }))} placeholder="SUP-001" />
            ...
            <Field label="العنوان" value={form.address ?? ''} onChange={(e) => setForm((p) => ({ ...p, address: e.target.value }))} />
